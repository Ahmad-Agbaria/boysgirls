...
$(document).ready(function () {
  //$('.no-sign-in-alert-modal').modal('show');
  //$('.forgot-password-modal').modal('show');
  //$('.new-password-modal').modal('show');
  //$('.how-to-play-modal').modal('show');
  //$('.game-terms-modal').modal('show');
  //$('.scoreboard-modal').modal('show');
});

var swiper = new Swiper(".swiper-choose-player", {
  slidesPerView: "auto",
  spaceBetween: 24,
...
// DENIS PAGES.
var pageChosenPlayer = document.querySelector('#pageChosenPlayer');
var pageRegisterFormPage = document.querySelector("#pageRegisterFormPage");
var pageLoginFormPage = document.querySelector("#pageLoginFormPage");

// Splash Page
var pageSplash = document.querySelector('#pageSplash');
var pageSplashStartBtn = document.querySelector('#pageSplashStartBtn');
// --
...
toolsBox = {
  ...
  hideSplashScreen: function() {
      ...
      moveBackgroundPackshot('-100px');
    toolsBox.delay(function() {
      toolsBox.showPage(pageGameMenu);
      toolsBox.hidePage(pageSplash);
    }, 500); // Show after 1.5s because the fadeOut-animation takes 0.5s and has 1s delay
  },
...
var circlesEngine = {
  create: function(typeOfCircle, numOfCircles) {
    ...
    switch (typeOfCircle.toLowerCase()) {
      case ".evil-circle":
        element.setAttribute('class', 'tpbl-circle c-red evil-circle donut donut-special');
      ...
      case ".good-circle":
            element.setAttribute('class', 'tpbl-circle c-blue good-circle donut ' + (_isBoy ? 'donut-green' : 'donut-purple'));

            var characterPic = document.createElement('img');
            characterPic.setAttribute('src', _characterPicUrl);
            element.appendChild(characterPic);
  ...
  addWithDelay: function(i, circle, typeOfCircle) { // add CSS class with delay
    ...
    }, i*0); // delay each using the index (i) * 50ms
  },
...
var gameEngine = { 
  ...
  levelPassed: function() {
    ...
    lvlPssdTtl.innerHTML = gameEngine.levelNum;
...
levelsEngine = {
  levels : [
    {
      levelNum: 1,
      time: 5, // Time in seconds for the current level
      tapValue: 3,
      tapsGoal: 5,
      goodCirclesCount: 1,
      evilCirclesCount: 4
...
var audioPool = {
  ...
  addSounds: function() {
    ...
    playSound: function (soundName) {
        if (document.hasFocus() && document.visiblityState !== 'hidden') {
...
toolsBox.startGameImmidiate = function() {
...
// -- Start Button
pageSplashStartBtn.addEventListener('click', function () {
    audioPool.playSound(buttonTap);
